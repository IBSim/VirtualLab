...
def Example():
    ...
    Parameters.TurnDiam = 0.0 #Pipe outer diameter in turned section
    Parameters.TurnLength = 0.0 #Length of turned section
...
def Create(Parameters):
    ...
    Pipe_tmp = geompy.MakeCutList(PipeExt, [Fluid], True)

    if Parameters.TurnDiam>0 and Parameters.TurnLength>0:
      ## Turned down pipe region (left hand side)
      TurnRad_L = Parameters.TurnDiam/2
      TurnLen_L = Parameters.TurnLength

      Vertex_2 = geompy.MakeVertex(Parameters.BlockWidth/2+Parameters.PipeCentre[0], -0.5*(Parameters.PipeLength-Parameters.BlockLength), Parameters.BlockHeight/2+Parameters.PipeCentre[1])
      TurnInner_L = geompy.MakeCylinder(Vertex_2, OY, TurnRad_L, TurnLen_L)
      TurnExt_L = geompy.MakeCylinder(Vertex_2, OY, OutRad, TurnLen_L)
      Turn_L = geompy.MakeCutList(TurnExt_L, [TurnInner_L], True)
      Pipe_tmp2 = geompy.MakeCutList(Pipe_tmp, [Turn_L], True)
    
      ## Turned down pipe region (right hand side)
      ## Assumed that both sides are symmetric
      ## This can be overridden if needed
      TurnRad_R = TurnRad_L
      TurnLen_R = TurnLen_L

      Vertex_3 = geompy.MakeVertex(Parameters.BlockWidth/2+Parameters.PipeCentre[0], 0.5*(Parameters.PipeLength+Parameters.BlockLength)-TurnLen_R, Parameters.BlockHeight/2+Parameters.PipeCentre[1])
      TurnInner_R = geompy.MakeCylinder(Vertex_3, OY, TurnRad_R, TurnLen_R)
      TurnExt_R = geompy.MakeCylinder(Vertex_3, OY, OutRad, TurnLen_R)
      Turn_R = geompy.MakeCutList(TurnExt_R, [TurnInner_R], True)
      Pipe = geompy.MakeCutList(Pipe_tmp2, [Turn_R], True)
    ...
    # Vertices
    GrpPipeInV1 = SalomeFunc.AddGroup(Sample, 'PipeInV1', [34])

    GrpPipeInV2 = SalomeFunc.AddGroup(Sample, 'PipeInV2', [45])

    GrpPipeOutV1 = SalomeFunc.AddGroup(Sample, 'PipeOutV1', [39])

    GrpPipeOutV2 = SalomeFunc.AddGroup(Sample, 'PipeOutV2', [50])

    # Create groups for surfaces of the sample include additional surfaces
    # created when parts join
    # Pipe
    # PipeIntIx = SalomeFunc.ObjIndex(Sample, Pipe, [51,41,46])[0]
    PipeIntIx = [51,41,46]
    PipeExtIx = [4,21,31,16,26,36]
