...
This is called in Methods/paraview.py
'''
...
def Run(file_name, func_name, fnc_args = {},fnc_kwargs = {}, ContainerInfo = None, tempdir='/tmp'):
    ...
    if ContainerInfo is None:
        ...
        ContainerInfo = GetInfo('paraview') 
    pth = "{}/{}.pkl".format(tempdir,uuid.uuid4())
    with open(pth,'wb') as f:
        pickle.dump((fnc_args,fnc_kwargs),f)
    vlab_dir = Utils.get_vlab_dir()
    container_bash = "{}/paraviewExec.sh".format(Dir) # bash script executed by container
    container_command = "python3 {}/bin/run_pyfunc.py {} {} {}".format(vlab_dir,file_name,func_name,pth)
