...
def bind_points2file(bind_points_dict :dict,vlab_dir :str):
    """
    Function to take the final list of container bind points used at runtime
    and outputs it to a json file.
    
    Param: bind_points_dict - python dict containing directories on the host
    as keys and the corresponding dir on the container as the values. 
    This is generally generated by VL_server.py from the combination of 
    bind_points_default, vlconfig and cmd options.

    Pram: vlab_dir - string or pathlike object pointing to the virtualLab directory
    """

    with open(f"{vlab_dir}/Config/bind_points_runtime.json", "w") as fp:
        json.dump(bind_points_dict , fp)
    return