...
.. note:: Creating images using **ParaVis** will produce 'Generic Warning' messages in the terminal. There are caused by bugs within the **SALOME** and can be ignored.
...
The first shows the normalised temporal laser profile used in the simulation, set in *Sim.LaserT*. The plot :file:`FluxDist.png` shows the spatial laser profile used (left), set by *Sim.LaserS*, and the loads that are applied to each node to produce this sitribution (right). 
